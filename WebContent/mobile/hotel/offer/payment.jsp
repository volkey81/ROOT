...
// 	request.setCharacterEncoding("utf-8");
	 ...
	 */
	String LGD_RETURNURL		= (SystemChecker.isReal() ? "https" : "http") + "://" + request.getServerName() + "/order/xpay/mreturnurl_hotel_offer.jsp";// FOR MANUAL
...
<script>
	...
	function getTxnId(){
        ...
        document.orderForm.action = "<%= Env.getSSLPath() %>/hotel/offer/proc.jsp";
        document.orderForm.acceptCharset = "utf-8";
        if (orderForm.canHaveHTML) { // detect IE
...
//		var Params = "customerOrderNumber=<%= orderid %>";
		...
		$.ajax({
			type: "POST",
			url: "/order/payco/payco_reserve_hotel_offer.jsp",
...
%>
					...
					<input type="hidden" name="naver_return" id="naver_return" value="mreturn_hotel_offer" />
...
<form name="paycoReserveForm" id="paycoReserveForm" method="post" action="/order/payco/payco_reserve.jsp">
	...
	<input type="hidden" name="returnUrl" value="payco_return_mobile_hotel_offer.jsp?orderid=<%= orderid %>">
</form>
<!-- KAKAOPAY Hidden -->
