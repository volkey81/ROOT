		...
		<div class="reservation">
			...
			<div class="completeArea">
				<div class="top">
					...
					<p class="num">예약번호 : <span><%= info.get("orderid") %></span></p>
				</div>
				<ul class="caption">
...
%>
				...
				<table class="allPriceList">
					...
					<tr>
						...
						<td><div><span><%= Utils.formatMoney(info.get("default_amt", "0")) %>원</span></div></td>
					</tr>
					<tr>
						<th scope="col">쿠폰 할인</th>
						<td><div><span>-<%= Utils.formatMoney(info.get("coupon_amt", "0")) %>원</span></div></td>
					</tr>
					<tr>
						<th scope="col">프로모션 코드 할인</th>
						<td><div><span>-<%= Utils.formatMoney(info.get("promocd_amt", "0")) %>원</span></div></td>
					</tr>
					<tr>
						<th scope="col">매일 Do 포인트</th>
						<td><div><span>-<%= Utils.formatMoney(info.get("point_amt", "0")) %>P</span></div></td>
					</tr>
					<tr>
						<th scope="col">기프트 카드</th>
						<td><div><span>-<%= Utils.formatMoney(info.get("giftcard_amt", "0")) %>원</span></div></td>
					</tr>
					<tr>
						<th scope="col">스페셜오퍼</th>
						<td><div><span>-<%= Utils.formatMoney(info.getInt("default_amt", 0) - info.getInt("tot_amt", 0)) %>원</span></div></td>
					</tr>
					<tr>
						<th scope="col">총 할인액</th>
						<td><div><span>-<%= Utils.formatMoney(info.getInt("coupon_amt") + info.getInt("promocd_amt") + info.getInt("point_amt") + info.getInt("giftcard_amt") + (info.getInt("default_amt") - info.getInt("tot_amt"))) %>원</span></div></td>
					</tr>
				</table>
				...
				<div class="btnArea">
					...
					<a href="/mobile/mypage/reservation/offer/list.jsp" class="btnStyle01 sizeM">예약 내역 조회</a>
				</div>
			</div>			
		</div>
