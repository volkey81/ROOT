...
%>
	...
	function showCartDone(_type){
		...
		$("#cartForm").attr("action", "/mobile/order/cartProc.jsp");
...
<%
		if(fs.isLogin()) {
%>
					parent.document.location.href="/mobile/order/payment.jsp";
<%
		} else {
%>
	...
	<!-- 내용영역 -->
		...
		<table class="bbsForm typeB optionForm">
			...
			<tr class="divide">
				...
				<td>
					<p class="countNum">
						<a href="#none" onclick="setQty('dn')"><img src="/mobile/images/btn/btn_minus.png" alt="-"></a>
						<input type="text" name="qty_<%= param.get("pid") %>" id="qty" value="<%= qty %>" readonly>
						<a href="#none" onclick="setQty('up')"><img src="/mobile/images/btn/btn_plus.png" alt="+"></a>
					</p>
				</td>
			</tr>
			<tr>
				...
				<td>
					<p class="countNum">
						<a href="#none" onclick="setRoutineCnt('dn')"><img src="/mobile/images/btn/btn_minus.png" alt="-"></a>
						<input type="text" name="routine_cnt" id="routine_cnt" value="<%= routineCnt %>" readonly>
						<a href="#none" onclick="setRoutineCnt('up')"><img src="/mobile/images/btn/btn_plus.png" alt="+"></a>
					</p>
				</td>
			</tr>
			<tr>
				...
				<td>
					<p class="dib"><input type="radio" name="routine_period" value="1" id="period1" <%= "1".equals(routinePeriod) ? "checked" : "" %>><label for="period1">매주</label></p>
					<p class="dib"><input type="radio" name="routine_period" value="2" id="period2" <%= "2".equals(routinePeriod) ? "checked" : "" %>><label for="period2">2주</label></p>
					<p class="dib"><input type="radio" name="routine_period" value="3" id="period3" <%= "3".equals(routinePeriod) ? "checked" : "" %>><label for="period3">3주</label></p>
					<p class="dib"><input type="radio" name="routine_period" value="4" id="period4" <%= "4".equals(routinePeriod) ? "checked" : "" %>><label for="period4">4주</label></p>
				</td>
			</tr>
			<tr>
...
%>
					<p class="dib"><input type="checkbox" name="routine_day" id="day_<%= row.get("day") %>" value="<%= row.get("day") %>" <%= routineDay.indexOf(row.get("day")) != -1 ? "checked" : "" %> onclick="calculateDate()"><label for="day_<%= row.get("day") %>"><%= row.get("day_name") %></label></p>
<%
	}
%>
				</td>
...
%>
			...
			<span><a href="#none" onclick="showCartDone();return false;" class="btnTypeB sizeL">적용</a></span>
<%
	} else {
%>
			<span><a href="#none" onclick="showCartDone();return false;" class="btnTypeA sizeL">장바구니 담기</a></span>
			<span><a href="#none" onclick="showCartDone('d');return false;" target="_parent" class="btnTypeB sizeL">바로구매</a></span>
<%
	}
%>
		</div>
