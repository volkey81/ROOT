...
%>
					showPopupLayer('/mobile/popup/memberGate.jsp', '580');
					hidePopupLayer(<%=layerId%>, 'reset');
<%
	}
%>
				} else {
					showPopupLayer('/mobile/popup/cartComplete.jsp', '580');
					hidePopupLayer(<%=layerId%>, 'reset');
				}
		...
		<table class="bbsList">
			...
			<tbody>
				<tr>
					...
					<td class="tit">
						<label for="sub_pid_<%= info.get("pid") %>" <%= isSoldOut ? "class=\"disabled\"" : "" %>><%= isSoldOut ? "[품절]" : "" %> <%= info.get("opt_pnm") %></label><br>
						<p class="countNum">
							...
							<a href="#none" onclick="setQty('down', '<%= info.get("pid") %>'); return false;"><img src="/mobile/images/btn/btn_minus.png" alt="-"></a>
							<input type="text" name="qty_<%= info.get("pid") %>" id="qty_<%= info.get("pid") %>" value="1">
							<a href="#none" onclick="setQty('up', '<%= info.get("pid") %>'); return false;"><img src="/mobile/images/btn/btn_plus.png" alt="+"></a>
						</p>
					</td>
				</tr>
<%
...
%>					
				<tr>
					...
					<td class="tit">
						<label for="sub_pid_<%= row.get("opt_pid") %>" <%= isSoldOut ? "class=\"disabled\"" : "" %>><%= isSoldOut ? "[품절]" : "" %> <%= row.get("opt_pnm") %></label><br>
						<p class="countNum">
							...
							<a href="#none" onclick="setQty('down', '<%= row.get("opt_pid") %>'); return false;"><img src="/mobile/images/btn/btn_minus.png" alt="-"></a>
							<input type="text" name="qty_<%= row.get("opt_pid") %>" id="qty_<%= row.get("opt_pid") %>" value="1">
							<a href="#none" onclick="setQty('up', '<%= row.get("opt_pid") %>'); return false;"><img src="/mobile/images/btn/btn_plus.png" alt="+"></a>
						</p>
					</td>
				</tr>
<%
...
%>
		...
		<div class="btnArea">
			<span><a href="#none" onclick="addCart('N')" class="btnTypeA sizeL">장바구니 담기</a></span>
			<span><a href="#none" onclick="addCart('Y')" class="btnTypeB sizeL">바로구매</a></span>
		</div>
		</form>
	<!-- //내용영역 -->
	</div><!-- //popCont -->
