...
<%
     ...
     */  
    String LGD_MERTKEY = Config.get("lgdacom.LGD_MERTKEY"); //mertkey
     ...
     */    
    String resultMSG = "sanghafarmshop ------------- 결제결과 상점 DB처리(LGD_CASNOTEURL) 결과값을 입력해 주시기 바랍니다.";  
    ...
    if (LGD_HASHDATA2.trim().equals(LGD_HASHDATA)) { //해쉬값 검증이 성공이면
        if ( ("0000".equals(LGD_RESPCODE.trim())) ){ //결제가 성공이면
        	if( "R".equals( LGD_CASFLAG.trim() ) ) {
                ...
                resultMSG = "OK";   
        		System.out.println("------ cas_noturl.jsp : " + LGD_OID + " : R");
            	...
            	//if( 무통장 입금 성공 상점처리결과 성공 ) 
           		resultMSG = "OK";
        		System.out.println("------ cas_noturl.jsp : " + LGD_OID + " : I");
 	            ...
 	            order.modifyStatus(p);
 	            
 	            try {
	 	            TmsUtil tms = new TmsUtil();
	 	            tms.sendOrderSms(LGD_OID, 1);
 	            } catch(Exception e) {
 	            	e.printStackTrace();
 	            }
            	...
            	resultMSG = "OK";
        		System.out.println("------ cas_noturl.jsp : " + LGD_OID + " : C");
         ...
         */      
        resultMSG = "sanghafarmshop ------------- 결제결과 상점 DB처리(LGD_CASNOTEURL) 해쉬값 검증이 실패하였습니다.";     
    }
