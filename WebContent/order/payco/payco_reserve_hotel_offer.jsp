...
// 	orderInfo.put("nonBankbookDepositInformUrl", nonBankbookDepositInformUrl); 	//[선택]무통장입금완료 통보 URL
	...
	if(isMobile) {
		if(SystemChecker.isReal()) {
			extraData.put("cancelMobileUrl", "https://www.sanghafarm.co.kr/mobile/hotel/offer/list.jsp");					//[선택][모바일 일경우 필수]모바일 결제페이지에서 취소 버튼 클릭시 이동할 URL
		} else if(SystemChecker.isDev()) {
			extraData.put("cancelMobileUrl", "http://dev.sanghafarm.co.kr/mobile/hotel/offer/list.jsp");					//[선택][모바일 일경우 필수]모바일 결제페이지에서 취소 버튼 클릭시 이동할 URL
		} else {
			extraData.put("cancelMobileUrl", "http://local.sanghafarm.co.kr/mobile/hotel/offer/list.jsp");					//[선택][모바일 일경우 필수]모바일 결제페이지에서 취소 버튼 클릭시 이동할 URL
		}
	}
