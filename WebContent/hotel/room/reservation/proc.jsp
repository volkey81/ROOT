...
<%
	...
	String backUrl = Env.getURLPath() + ("P".equals(deviceType) ? "" : "/mobile") + "/hotel/room/reservation/date.jsp";

	if(list.size() == 0) {
		Utils.sendMessage(out, "선택하신 날짜의 숙박 가능한 객실이 없습니다.", backUrl);
		return;
	}
	...
	List<Param> roomList = new ArrayList<Param>();
			...
			if(adult + kids > 0) {
				if(adult + kids > basic) {	// 추가요금 계산
					if(adult > basic) {	// 성인만으로도 기본 초과인 경우
						...
						_addAmt += (_addAdultAmt * (adult - basic) * night);
						if(kids > 0) {
							addKids += kids;
							_addAmt += (_addChildAmt * kids * night);
						}
					} else {
						addKids += (adult + kids - basic);
						_addAmt += (_addChildAmt * (adult + kids - basic) * night);
					}
				}
				...
				Param p = row.copy();
				p.set("pers_adlt", adult);
				p.set("pers_kids", kids);
				p.set("add_amt", _addAmt);
				p.set("rem_cntn", param.get(row.get("room_knd_gbcd") + "_note"));
				roomList.add(p);
			}
		}
		
		if(roomCnt > 0) {
			if(row.getInt("forcasting_qty") < roomCnt) {
				System.out.println(orderid + "- 객실 부족 " + row.get("room_knd_gbcd") + " " + roomCnt + " : " + row.getInt("forcasting_qty"));
				Utils.sendMessage(out, "선택하신 날짜의 숙박 가능한 객실이 부족합니다.", backUrl);
				return;
			}
...
// 	boolean nopg = true;
	...
	if(param.getInt("LGD_AMOUNT") == 0 || (nopg && SystemChecker.isLocal())) {	// 0원 결제
		try {
			svc.create(request, response, roomList);
		} catch(Exception e) {
			e.printStackTrace();
	    ...
	    if(responseCode == 200 && "Success".equals(code)) {	// 결제성공
			...
			try {
				...
				svc.create(request, response, roomList, payInfo);
	    	} catch(Exception e) {
	    		e.printStackTrace();
	     ...
	     if ( xpay.TX() ) {
	         ...
	         if( "0000".equals( xpay.m_szResCode ) ) {
	         	...
	         	try {
					...
					svc.create(request, response, roomList, lgdInfo);
	         	} catch(Exception e) {
	...
	try {
		...
		content += "예약번호 : " + info.get("intg_resv_no") + "\n";
	...
	nextUrl += "/hotel/room/reservation/complete.jsp";
%>
