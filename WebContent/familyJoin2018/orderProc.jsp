...
<%
	...
	String joinid = param.get("joinid");
	...
	*/
	...
	String recommender = SanghafarmUtils.getCookie(request, "RECOMMENDER2");
	
	param.set("recommender", recommender);
	param.set("tot_amt", price);
	param.set("coupon_amt", "0");
	param.set("pay_amt", price);
	param.set("point_amt", "0");
	param.set("status", "1");
	...
	if("".equals(param.get("userid"))) {
		...
		if(recommender.equals(fs.getUserNo() + "")) {
			param.set("recommender", "");
		}
	} else {	// 선물하기
		param.set("userid", param.get("userid"));
		param.set("unfy_mmb_no", param.get("userno"));
		param.set("path", "4");
		param.set("giver", fs.getUserNo());
		param.set("recommender", "");
	}
	...
	if(!"".equals(param.get("mem_couponid"))) {
		...
		if(couponInfo == null) {
			...
			if(price < couponInfo.getInt("min_price", 0)) {
			...
			if("A".equals(couponInfo.get("sale_type"))) {	//  정량
				...
				couponAmt = (couponInfo.getInt("max_sale") <= (price * couponInfo.getInt("sale_amt") / 100)) ? 
						couponInfo.getInt("max_sale") : (price * couponInfo.getInt("sale_amt") / 100);
			}
		}

		param.set("pay_amt", price - couponAmt);
	}

	System.out.println("PAY_AMT : " + (price - couponAmt) + " : " + param.get("LGD_AMOUNT"));
	if((price - couponAmt) != param.getInt("LGD_AMOUNT")) {
		System.out.println("----------------- invalid order 유효하지 않은 주문서입니다.(6)");
		Utils.sendMessage(out, "유효하지 않은 주문서입니다.(6)", backUrl);
		return;
	}

	if(param.getInt("LGD_AMOUNT") == 0 || (SystemChecker.isLocal() && !"010".equals(param.get("pay_type")))) {	// 0원 결제
		try {
			svc.onJoin(param);
		} catch(Exception e) {
...
// 			    order.createKakaoPayCancelLog(payInfo);
	...
	nextUrl += "/familyJoin2018/complete.jsp";
	
	SanghafarmUtils.setCookie(response, "RECOMMENDER", "", fs.getDomain(), 60*60*24*300);
	SanghafarmUtils.setCookie(response, "RECOMMENDER2", "", fs.getDomain(), 60*60*24*300);
%>
<form name="orderForm" id="orderForm" action="<%= nextUrl %>" method="POST">
	<input type="hidden" name="orderid" value="<%= param.get("orderid") %>" />
	<input type="hidden" name="path" value="<%= param.get("path") %>" />
</form>
