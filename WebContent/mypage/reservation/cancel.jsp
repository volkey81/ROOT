...
%>
					<tr>
						<th scope="row"><%= row.get("order_date") %><p class="fs"><a href="view.jsp?orderid=<%= row.get("orderid") %>" class="fontTypeC">(<%= row.get("orderid") %>)</a></p></th>
...
%>
						...
						<td><%= row.get("reserve_date") %><p class="fontTypeC"><%= !"".equals(row.get("time")) ? row.get("time").substring(0, 2) + ":" + row.get("time").substring(2) : "" %></p></td>
						<td><%= row.get("cancel_date") %></td>
