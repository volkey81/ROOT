...
<div id="popWrap">
	...
	<!-- 내용영역 -->
		...
		<table class="bbsList">
			...
			<colgroup>
				<col width="80"><col width="100"><col width="280"><col width="">
			</colgroup>
			<thead>
				<tr>
					<th scope="col">선택</th>
		...
		<div class="scrTable addressList">
			<table class="bbsList">
				<caption>배송주소선택</caption>
				<colgroup>
					<col width="80"><col width="100"><col width="280"><col width="">
...
<script>
	...
	addrs[<%= i %>] = addr;
					...
					<tr>
						<th scope="row"><input type="radio" name="seq" id="addr_<%= i %>" value="<%= row.get("seq") %>"></th>
						<td><label for="addr_<%= i %>"><%= Utils.safeHTML(row.get("addr_name")) %></label></td>
						<td class="tit">
							<strong><%= Utils.safeHTML(row.get("name")) %></strong><br>
						...
						<td>
							<a href="#none" onclick="setAddr(<%= i %>)" class="btnA">선택</a>
							<a href="#none" onclick="removeAddr('<%= row.get("seq") %>')" class="btnB">삭제</a>
						</td>
					</tr>
<%
	}
	...
	if(list.size() == 0) {
%>
					<tr><td colspan="4">+++ 선택가능한 배송지가 없습니다 +++</td></tr>
<%
	}
%>
				</tbody>
		...
		<div class="btnArea">
			<a href="javascript:setDefault()" class="btnTypeB sizeL">기본 배송지로 선택</a>
			<a href="#none" onclick="showPopupLayer('/popup/newAddress.jsp', '630'); hidePopupLayer(<%=layerId%>, 'reset'); return false" class="btnTypeA sizeL">신규배송지 등록</a>
		</div>
	<!-- //내용영역 -->
	</div><!-- //popCont -->
</div><!-- //popWrap -->
