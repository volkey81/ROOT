...
<div id="popWrap">
	...
	<!-- 내용영역 -->
		...
		<table class="bbsForm typeB">
			...
			<tr>
				<th scope="row">제목</th>
				<td><input type="text" name="title" id="title" title="제목" style="width:420px" value="<%= info.get("title") %>"></td>
			</tr>
			<tr>
				<th scope="row">사진첨부</th>
				<td>
					<p class="fileBox">
						<input type="text" id="file1" readonly="readonly" style="width:340px">
						<span class="file btnTypeB">파일첨부<input type="file" id="img1" name="img1" onchange="$('#file1').val($('#img1').val())"></span>
						<input type="hidden" name="img1_org" value="<%= info.get("img1") %>">
					</p>
					<p class="fileBox">
						<input type="text" id="file2" readonly="readonly" style="width:340px">
						<span class="file btnTypeB">파일첨부<input type="file" id="img2" name="img2" onchange="$('#file2').val($('#img2').val())"></span>
			...
			<tr>
				<th scope="row">본문</th>
				<td><textarea name="contents" id="contents" style="width:420px;height:80px" title="본문"><%= info.get("contents") %></textarea></td>
			</tr>
		</table>
		</form>
		<div class="btnArea">
			<a href="#" onclick="hidePopupLayer(); return false" class="btnTypeA sizeL">취소</a>
			<a href="#" onclick="goSubmit(); return false" class="btnTypeB sizeL"><%= StringUtils.equals("CREATE", mode) ? "등록" : "수정"%></a>
		</div>
	<!-- //내용영역 -->
	</div><!-- //popCont -->
</div><!-- //popWrap -->
