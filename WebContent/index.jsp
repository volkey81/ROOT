...
<%
	...
	if("A".equals(fs.getDeviceType())) {
		...
		if(isMobileOS) {
			response.sendRedirect("/mobile/?" + request.getQueryString());
		} else {
			String landing = SanghafarmUtils.getCookie(request, "LANDING_PAGE", "BRAND");
			
			if("BRAND".equals(landing)) {
				response.sendRedirect("/brand/?" + request.getQueryString());
			} else if("HOTEL".equals(landing)) {
					response.sendRedirect("/hotel/?" + request.getQueryString());
			} else {
				response.sendRedirect("/main.jsp?" + request.getQueryString());
			}
		}
	}
%>
